...
class BaseATSScraper(BaseScraper):
    def __init__(self, base_url, params, validation_queue: Queue, enrichment_queue: Queue, config: ScraperConfig | None = None, base_ats_fetch_semaphore: int = 5):
        super().__init__(
        ...
        self.params = params
        self.base_ats_fetch_semaphore = asyncio.Semaphore(base_ats_fetch_semaphore)
    ...
    async def fetch(self, company_name: str, client: httpx.AsyncClient) -> int:
        async with self.base_ats_fetch_semaphore:
